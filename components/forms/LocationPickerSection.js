// 🎯 Ensure these are imported if this is in a separate file!
...
export const LocationPickerSection = ({
  ...
  // 🛡️ Safe extraction using getIn for nested Formik paths
  const gpsValue = values.ast?.location?.gps;
  const gpsError = getIn(errors, "ast.location.gps");
  const hasError = !!gpsError;

  return (
    <FormSection title="Meter Location">
      <Text style={[styles.label, hasError && { color: "#ef4444" }]}>
        Physical Asset Positioning
      </Text>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
          <View
            style={[
              ...
              { backgroundColor: gpsValue?.lat ? "#dcfce7" : "#fee2e2" }, // 🎯 Check for .lat specifically
            ]}
          >
            <MaterialCommunityIcons
              name={gpsValue?.lat ? "map-marker-check" : "map-marker-alert"}
              size={24}
              color={gpsValue?.lat ? "#16a34a" : "#dc2626"}
            />
          </View>
          ...
          <View style={{ marginLeft: 12 }}>
            <Text
              style={[
                ...
                { color: gpsValue?.lat ? "#1e293b" : "#dc2626" },
              ]}
            >
              {/* 🎯 Use Optional Chaining (?.) to prevent crashes if lat/lng are missing */}
